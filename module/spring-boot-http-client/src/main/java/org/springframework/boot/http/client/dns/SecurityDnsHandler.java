...
/**
 * Component to filter addresses according to configured security rules.
 * For use from an HTTP client's DNS resolver.
 *
...
public final class SecurityDnsHandler {
    ...
    public List<SocketAddress> handleSocketAddresses(List<? extends SocketAddress> candidates) {
        ...
        return new ArrayList<SocketAddress>(
                candidates.stream().filter(sa -> !(sa instanceof InetSocketAddress) || filteredInet.contains(sa)).toList());
    }
