...
/**
 * Default implementation of {@link InetAddressFilter} that provides SSRF protection by
 * filtering addresses based on allow/deny lists and internal/external network rules.
 *
 * @author KJ202
 * @since 3.4.0
 */
public final class DefaultInetAddressFilter implements InetAddressFilter {
    ...
    private static final Log logger = LogFactory.getLog(DefaultInetAddressFilter.class);
    ...
    public DefaultInetAddressFilter(List<String> allowList, List<String> denyList, boolean blockExternal,
        ...
        if (!allowList.isEmpty() && !denyList.isEmpty()) {
            throw new IllegalArgumentException("Logic inconsistency: allowList and denyList cannot be used at the same time");
        }
    ...
    private static List<IpAddressMatcher> initIpList(List<String> ipList) {
        return ipList.stream().map(IpAddressMatcher::new).collect(Collectors.toList());
    }
    ...
    /**
     * Enum to represent the block mode for address filtering.
     */
    private enum BlockMode {
        ...
        /**
         * Allow requests towards the internet only, e.g. prevent access to cloud VM metadata.
         */
        INTERNAL;
    ...
    /**
     * Class to represent an IPv4 or IPv6 range to be used in filtering.
     */
    private static final class IpAddressMatcher {
        ...
        public String toString() {
            return "IpAddressMatcher{address=" + address + ", nMaskBits=" + nMaskBits + '}';
        }
